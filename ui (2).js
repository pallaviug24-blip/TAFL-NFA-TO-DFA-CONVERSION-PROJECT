  ...
  return [
    ...
    },
    {
      ...
      style: {
        ...
        'text-margin-y': -10,
        'text-background-color': isDark ? '#0d0f17' : '#f4f6fc',
...
function renderGraph(graphType) {
  ...
  // Use breadthfirst for small, directed DFA/minDFA (clean left-to-right flow)
  const useDAG = (graphType === 'dfa' || graphType === 'min') && nodeCount <= 12;
  // Use cose-bilkent if loaded (better physics), else fall back to cose
  let coseName = 'cose';
  try { if (cytoscape('layout', 'cose-bilkent')) coseName = 'cose-bilkent'; } catch (e) {}

  const layoutConfig = useDAG
    ? {
        name: 'breadthfirst',
        directed: true,
        animate: true,
    ...
    : {
        ...
        idealEdgeLength: () => 160,
        // Push non-connected nodes apart
        nodeOverlap: 20,
        ...
        numIter: 2500,
        // Initial temperature (controls random movement)
        initialTemp: 200,
        coolingFactor: 0.95,
        minTemp: 1.0,
        // Respect node sizes when calculating overlap
        nodeDimensionsIncludeLabels: true,
        randomize: false,
        componentSpacing: 80
      };
...
function updateAllTables() {
  document.getElementById('table-nfa').innerHTML =
    state.nfa ? AE.buildNFATableHTML(state.nfa) : '<p class="placeholder-text">No NFA built yet.</p>';
...
document.getElementById('buildNfa').addEventListener('click', () => {
  ...
  document.getElementById('nfa-table-preview').innerHTML = AE.buildNFATableHTML(nfa);
...
document.getElementById('resetAll').addEventListener('click', () => {
  ...
  document.getElementById('nfa-table-preview').innerHTML = '<p class="placeholder-text">Build an NFA to see the transition table.</p>';
...
document.getElementById('layoutGraph').addEventListener('click', () => {
  if (!state.cy) return;
  const nodeCount = state.cy.nodes().length;
  const useDAG = (state.currentGraph === 'dfa' || state.currentGraph === 'min') && nodeCount <= 12;
